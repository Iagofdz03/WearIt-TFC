...
public class OutfitSugerenciaService {
    ...
    public List<SugerenciaOutfitDTO> sugerirOutfitsConFiltros(
    	    Long usuarioId, String ocasion, String temporada) {
    	    
    	  List<Prenda> todasPrendas = prendaRepository.findByUsuarioId(usuarioId);
    	  
    	  // Filtrar por temporada si se indica
    	  List<Prenda> prendas = todasPrendas.stream()
    	    .filter(p -> temporada == null || 
    	                 "todo año".equalsIgnoreCase(p.getTemporada()) || 
    	                 temporada.equalsIgnoreCase(p.getTemporada()))
    	    .collect(java.util.stream.Collectors.toList());
    	  
    	  if (prendas.isEmpty()) prendas = todasPrendas; // fallback
    	  
    	  List<SugerenciaOutfitDTO> sugerencias = new ArrayList<>();
    	  
    	  // Si hay filtro de ocasión, generar outfit por estilo/ocasión
    	  if (ocasion != null && !ocasion.isEmpty()) {
    	    List<Prenda> outfitOcasion = generarOutfitPorEstilo(prendas, ocasion);
    	    if (outfitOcasion.size() >= 2) {
    	      sugerencias.add(new SugerenciaOutfitDTO(
    	        "Look " + capitalizar(ocasion) + 
    	          (temporada != null ? " · " + temporada : ""),
    	        outfitOcasion,
    	        "Selección para ocasión " + ocasion + 
    	          (temporada != null ? " en temporada " + temporada : "")
    	      ));
    	    }
    	  }
    	  
    	  // Completar con sugerencias base filtradas por temporada
    	  List<SugerenciaOutfitDTO> base = sugerirOutfits(usuarioId);
    	  sugerencias.addAll(base);
    	  
    	  return sugerencias.stream().distinct()
    	    .limit(5)
    	    .collect(java.util.stream.Collectors.toList());
    	}
